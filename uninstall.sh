...
do
	...
	if [[ ${osInfo[$f]} == "dnf" ]];then
		sudo dnf remove thunar swaybg hyprland waybar wofi wlogout swaylock SwayNotificationCenter pavucontrol kitty polkit-kde
	fi
	if [[ ${osInfo[$f]} == "yay" ]];then
		yay -R hyprland wofi swaync waybar wlogout swaylock swaybg kitty thunar xwaylandvideobridge polkit-kde-agent
	fi
