...
do
	...
	if [[ ${osInfo[$f]} == "dnf" ]];then
		...
		sudo dnf install thunar swaybg hyprland waybar wofi wlogout swaylock SwayNotificationCenter pavucontrol kitty polkit-kde
	fi
	if [[ ${osInfo[$f]} == "yay" ]];then
		yay -S hyprland wofi swaync waybar wlogout swaylock swaybg kitty thunar xwaylandvideobridge polkit-kde-agent
	fi
	if [[ ${osInfo[$f]} == "apt-get" ]];then
		sudo apt-get install -y meson wget build-essential ninja-build cmake-extras cmake gettext gettext-base fontconfig libfontconfig-dev libffi-dev libxml2-dev libdrm-dev libxkbcommon-x11-dev libxkbregistry-dev libxkbcommon-dev libpixman-1-dev libudev-dev libseat-dev seatd libxcb-dri3-dev libegl-dev libgles2 libegl1-mesa-dev glslang-tools libinput-bin libinput-dev libxcb-composite0-dev libavutil-dev libavcodec-dev libavformat-dev libxcb-ewmh2 libxcb-ewmh-dev libxcb-present-dev libxcb-icccm4-dev libxcb-render-util0-dev libxcb-res0-dev libxcb-xinput-dev xdg-desktop-portal-wlr libtomlplusplus3 sway-notification-center thunar wlogout wofi swaylock waybar swaybg kitty brightnessctl pamixer pavucontrol polkit-kde-agent
	fi
    fi
done
