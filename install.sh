...
do
	...
	if [[ ${osInfo[$f]} == "dnf" ]];then
		...
		sudo dnf install nautilus hyprland waybar wofi wlogout swaylock SwayNotificationCenter pavucontrol kitty polkit-kde
	fi
	if [[ ${osInfo[$f]} == "yay" ]];then
		yay -S nautilus hyprland wofi swaync waybar wlogout swaylock swaybg kitty xwaylandvideobridge polkit-kde-agent
	fi
